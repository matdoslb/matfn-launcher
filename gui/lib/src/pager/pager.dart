...
import 'package:reboot_launcher/src/config.dart'; // << added
...
class _RebootPagerState extends State<RebootPager>
    with WindowListener, AutomaticKeepAliveClientMixin {
  final BackendController _backendController = Get.find<BackendController>();
  final GameController _gameController = Get.find<GameController>();
  final HostingController _hostingController = Get.find<HostingController>();
  final ServerBrowserController _serverBrowserController =
      Get.find<ServerBrowserController>();
  ...
  final PageController _pageController =
      PageController(keepPage: true, initialPage: pageIndex.value);
  ...
  void _syncPageViewWithNavigator() {
    ...
    pageIndex.listen((index) {
      // clamp to visible range (Play/Settings)
      final safe = (index < 0 || index >= pages.length) ? 0 : index;
      if (safe == lastPage) {
        return;
      }
      lastPage = safe;
  ...
  Future<void> _checkGameServer() async {
    // In client builds, never overwrite the forced host
    if (kClient) return;
    ...
    try {
      ...
      // Intended behaviour: ignore
    }
  }

  void _checkUpdates() {
    checkLauncherUpdate(onUpdate: (latestVersion) {
  ...
  Widget _buildBodyContent() => PageView.builder(
        ...
        itemBuilder: (context, index) => Navigator(
          ...
          observers: [
            _NestedPageObserver(onChanged: (routeName) {
              if (routeName != null) {
                pageIndex.refresh();
  ...
  TextSpan _buildBodyHeaderNestedPage({
    ...
    return TextSpan(
      ...
      style: TextStyle(color: last ? null : inactiveColor),
    );
  }
  ...
  Widget _buildLateralView() => SizedBox(
        ...
        child: Column(
          ...
          children: [
            Obx(() {
              pageIndex.value;
              return ProfileWidget(overlayKey: profileOverlayKey);
            }),
            _autoSuggestBox,
          ...
          child: Scrollbar(
            child: ListView.separated(
              ...
              itemBuilder: (context, index) =>
                  _buildNavigationItem(pages[index]),
            ),
          ),
        ),
      );
  ...
  Widget _buildNavigationItem(AbstractPage page) {
    // visible index in trimmed `pages`
          ...
          () => Container(
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  SizedBox.square(
                      dimension: 24, child: Image.asset(page.iconAsset)),
        ...
        child: AutoSuggestBox<PageSuggestion>(
          ...
          trailingIcon: IgnorePointer(
            child: IconButton(
              onPressed: () {},
              icon: Transform.flip(
                  flipX: true, child: const Icon(FluentIcons.search)),
            ),
          ),
        ),
      );

  List<AutoSuggestBoxItem<PageSuggestion>> get _suggestedItems =>
      pages.mapMany((page) {
        final pageIcon =
            SizedBox.square(dimension: 24, child: Image.asset(page.iconAsset));
        ...
        results.add(
          AutoSuggestBoxItem(
            value: PageSuggestion(
              name: page.name,
              description: "",
              pageIndex: pages.indexOf(page), // 0..1
            ),
            label: page.name,
            child: pageIcon,
          ),
  ...
  void didPush(Route route, Route? previousRoute) {
    if (previousRoute != null) {
      WidgetsBinding.instance
          .addPostFrameCallback((_) => onChanged(route.settings.name));
    }
  }
}
